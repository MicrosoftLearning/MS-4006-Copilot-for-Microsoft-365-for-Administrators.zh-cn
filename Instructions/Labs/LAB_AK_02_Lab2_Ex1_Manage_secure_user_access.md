...
然后，Adatum 的 CTO 要求 Holly 部署 Microsoft Entra 多重身份验证 (MFA)、直通身份验证 (PTA) 和 Microsoft Entra 智能锁定。 这三项功能将有助于在整个组织中加强密码管理，为适用于 Microsoft 365 的 Copilot 做好准备。 对于 PTA，你将使用 Microsoft Entra Cloud Sync 部署它。对于智能锁定，你将使用组策略管理部署它。 

对于 MFA，你将创建一个条件访问策略，以便为 Adatum 的所有用户部署 MFA。 然后，你将对其进行修改，以排除 Holly 及其 Microsoft 365 试点项目团队的选定成员。 这样，你就无需在他们登录时使用 MFA，并获得有关如何在条件访问策略中排除用户的经验。 

**注意：** 排除特定用户对 MFA 的使用并不是你在实际方案中通常会执行的操作。 但是，为了节省本课堂培训实验室的时间，我们将为测试用户禁用 MFA。 
...
话虽如此，由于此培训课程时间有限，并且我们不希望占用实验室时间，强制你在每次 Holly 登录时都使用第二种身份验证方法登录，因此你将不必为 Holly 的账户启用 MFA。
...
### 任务 2 - 设置 Microsoft 365 用户帐户

完成上述任务后，你应仍以 MOD 管理员帐户身份登录到 Microsoft 365 管理中心。******** 在此任务中，作为 Adatum 的新 Microsoft 365 管理员 Holly Dickson，你将开始实现 Adatum 的 Microsoft 365 试点项目。 因此，在此任务中，你将首先以 MOD 管理员的身份退出登录 Microsoft 365，然后再使用 Holly 的新 Microsoft 365 帐户重新登录。 

在前面的任务中，你注意到你的 Microsoft 365 试用租户配备了活动用户列表。 作为 Adatum 的 Microsoft 365 管理员 Holly Dickson，你选择了 Microsoft 365 试点项目团队的以下成员，以帮助完成部署的初始阶段：Alex Wilber、Joni Sherman、Lynne Robbins、Patti Fernandez. 

每个用户都是你的试点项目团队的关键成员。 虽然他们的用户帐户已在 Microsoft 365 中存在，但你想要配置他们的密码，以便在即将到来的实验室练习中在需要时更轻松地登录到 Microsoft 365。 你还将更改 Adele Vance 的密码。 Adele 不是 Microsoft 365 试点项目团队的成员，但在后面的实验室中测试 MFA 实现时会用到她。 你将实验室托管提供商为租户管理员帐户（即 MOD 管理员帐户）提供的管理密码分配为他们的用户密码，就像创建 Holly 的帐户时所做的那样。****  

**重要提示：** 对多个用户使用相同的密码显然永远不应是现实中的做法。 但是，在你的培训环境中这样做只是为了让学生在实验室过程中更轻松一些。 话虽如此，此任务还将为你提供关于如何更改用户密码的经验。
...
5. 在“输入密码”窗口中，输入实验室托管提供商为租户管理员帐户（即 MOD 管理员帐户）提供的相同“管理密码”，然后选择“登录”。************

6. 如果“欢迎使用 Microsoft 365”对话框显示在屏幕中间，则无法将其关闭。**** 在窗口右侧，选择两次向前箭头图标 (**>**)，然后选择复选标记图标以继续浏览此消息窗口中的幻灯片。 

7. 如果显示了“使用 Microsoft 365 创建”窗口，请选择窗口右上角的 X 将其关闭。******** 
...
8. “欢迎使用 Microsoft 365”页面将显示在 Edge 浏览器的“主页 | Microsoft 365”选项卡中********。这是 Holly 的 Microsoft 365 主页。 请注意，Holly 的首字母缩写显示在屏幕右上角，但是，Holly 的姓名不会显示。 这是因为当你将 Microsoft 365 试点项目用户添加到与上一实验室练习中的自定义主题关联的组中时，Holly 的帐户不存在。 由于 Holly 希望在她登录到系统时在每个 Microsoft 365 窗口顶部都看到她的姓名，她首先想要将其帐户添加到 Microsoft 365 试点项目用户组。 <br>
    ...
    在屏幕最左侧显示的应用程序图标列中，选择“管理员”。**** 这会在新浏览器选项卡中打开 Microsoft 365 管理中心。**** 
...
12. 在“成员身份”选项卡中，默认情况下，“所有者”子选项卡显示在窗格左侧的导航窗格中。******** 选择其下方显示的“成员”子选项卡。****
...
正如你的培训所表明的，有三种方法来实现 MFA - 使用条件访问策略、使用安全默认值、使用旧版每用户 MFA（不建议用于大型组织）。 在本练习中，你将通过条件访问策略启用 MFA，这是 Microsoft 建议的方法。 Adatum 已指示 Holly 为所有 Microsoft 365 用户（包括内部和外部）启用 MFA。 但是，为了测试 Adatum 的 Microsoft 365 试点项目实施，Holly 希望排除 M365 试点项目组的成员，让他们不必使用 MFA 登录。 完成试点项目后，Holly 将通过从 MFA 要求中移除此组的排除来更新策略。 该策略还将包括另外两个要求。 它将要求对所有云应用使用 MFA，即使用户从受信任的位置登录，也需要 MFA。 

1. 在 LON-CL1 VM 中，Microsoft 365 管理中心应延续之前的任务，仍在 Microsoft Edge 浏览器中打开。**** 你应以 Holly Dickson 的身份登录到 Microsoft 365。****
   
2. 在 Microsoft 365 管理中心的导航窗格中的“管理中心”部分下，选择“标识”。************ 这样做会在新的浏览器选项卡中打开 Microsoft Entra 管理中心。 

3. 在 Microsoft Entra 管理中心，选择导航窗格中的“保护”，然后选择“条件访问”。************

4. 在“条件访问 | 概览”页，选择“策略”。********

5. 在“条件访问 | 策略”页，查看你的 Microsoft 365 订阅提供的默认策略。**** 在页面顶部的菜单栏中，选择“+新建策略”。****

6. 在“新建条件访问策略”窗口中，在“名称”字段中输入“对所有 Microsoft 365 用户要求 MFA”。************

7. 首先需定义用户的 MFA 要求。 在“用户”组下，选择“已选择 0 个用户和组”。******** 这样做会显示两个选项卡 -“包含”和“排除”。********

8. 在“包含”选项卡下，选择“所有用户”。******** 请注意显示的警告消息。 你将在接下来的两个步骤中解决此问题。

9. 选择“排除”选项卡。**** 为了避免系统锁定，如前面的警告消息所示，你需要排除你的全局管理员（在此例中，即 Holly）。 为了测试时方便起见，Holly 还希望排除其他 Microsoft 365 试点项目组成员。 Microsoft 365 上线后，Holly 将从此条件访问策略中的“排除”列表中移除试点项目组，只排除她自己和其他几个全局管理员。 但目前，Holly 想排除整个组。 <br/>

    为此，请选择“用户和组”。**** 

10. 在显示的“选择排除的用户和组”窗口中，需要选择 Microsoft 365 试点项目组。**** 默认显示“全部”选项卡。**** 若要快速查找试点项目组，请选择“组”选项卡。**** 在活动组列表中，选中“M365 试点项目”组旁边的复选框，然后选择窗口底部的“选择”按钮。******** 返回“新建条件访问策略”窗口，注意“用户”部分下显示的消息。******** 

11. 现在，你将为所有云应用定义 MFA 要求。 在“目标资源”部分下，选择“未选择目标资源”。******** 这样做会显示两个选项卡 -“包含”和“排除”。********

12. 在“包含”选项卡下，请注意，默认设置为“无”。******** 如果未更新此设置，则没有云应用需要 MFA，包括 Microsoft 365。 因此，即使你创建了此策略并在步骤 17 中选择了要求 MFA 的选项，但将此“目标资源”设置保留为“无”，则任何登录到 Microsoft 365 的用户都无需使用 MFA。******** <br/>

    选择“选择应用”选项。**** 在“选择”组下，选择“无”。******** 在显示的“选择云应用”窗格中，向下滚动应用列表以查看可以要求使用 MFA 的所有不同应用。**** 请勿选择任何应用。**** 我们让你滚动浏览此列表，只是为了让你了解当你决定限某些应用使用 MFA 时，在要求 MFA 方面可以达到的粒度。  <br/>

    对于 Adatum，Holly 希望对所有云应用要求 MFA，这通常是一种比选择特定应用更常见的业务方案。 在“包含”选项卡下，选择“所有云应用”选项。******** Adatum 将不会从 MFA 身份验证中排除任何云应用。 如果想要查看“排除”选项卡提供的选项，则可以选择它，但不要选择任何云应用进行排除。**** 

13. 最后，定义所有用户登录位置的 MFA 要求。 在某些情况下，组织可能只会在用户从不受信任的位置登录时才要求 MFA。 但是，对于所有包含的用户，无论他们从何处登录，Adatum 都将要求他们进行 MFA。 <br/>

    在“条件”下，选择“已选择 0 个条件”********。 这样做会显示策略将检查的潜在条件的列表。 对于本实验室练习，在“位置”条件下，选择“未配置”。******** 这样做会显示一个“配置”切换开关和两个选项卡 -“包含”和“排除”。************

14. 将“配置”切换开关设置为“是”，这将启用这两个选项卡。******** 

15. 在“包含”选项卡下，验证是否选择了“任何位置”（如有必要，请选择它）。******** 选择“排除”选项卡。**** 如果你的组织将特定的 IP 地址或地址范围识别为“受信任”，则如果用户从其中一个位置登录，可以排除 MFA 要求。 但是，Adatum 希望要求所有包含的用户登录使用 MFA，无论他们的位置在何处。 这包括内部和外部用户登录。验证是否已选择“所选位置”选项，然后在“选择”部分下，验证它是否显示“无”。************ 如果未指定任何所选位置，则此设置可确保不会从 MFA 中排除任何位置。 

16. 在“访问控制”部分的“授予”组下，选择“已选择 0 个控制”。************ 这样做会显示“授予”窗格。****

17. 在显示的“授予”窗格中，验证是否选择了“授予访问权限”选项（如有必要，请选择它）。******** 然后选中“要求多重身份验证”复选框。**** 请注意可以使用此策略启用的所有其他访问控制。 对于此策略，你将只要求 MFA。 选择“授予”窗格底部的“选择”按钮，关闭窗格。******** 

18. 在“新建”窗口底部的“启用策略”字段中，选择“开”。************

19. 请注意页面底部显示的选项，它警告你不要把自己锁在外面。选择选项“我了解我的帐户将受此策略的影响。**** 仍然继续。” 事实上，Holly 不会受到影响，因为她是 M365 试点项目组的成员，被排除在此策略之外。

20. 选择“创建”按钮以创建策略。****

21. 在显示的“条件访问 | 策略”窗口中，验证是否显示“要求所有 Microsoft 365 用户使用 MFA”策略，并且其“状态”是否设置为“开”。****************

22. 保持登录到 LON-CL1，让所有 Microsoft Edge 浏览器选项卡都开着，供下一个任务使用。

### 任务 4：为包含和排除的用户测试 MFA

若要测试刚刚创建的条件访问策略，需要以 Holly 的身份退出登录 Microsoft 365，然后以 Adele Vance 的身份重新登录。 Adele 不是 M365 试点项目组的成员，因此 Microsoft Entra 应该要求她在登录时使用 MFA。 以 Adele 的身份登录并验证 MFA 能够正常工作后，你将以 Adele 的身份退出登录，然后以 Holly 的身份重新登录。 由于 Holly 是 M365 试点项目组的成员，该组在条件访问策略中使用 MFA 的要求中被排除，因此以 Holly 的身份登录时，你应该不必使用 MFA。 

**重要提示：** 若要实现 MFA，需要使用手机来接收验证码，以便将其输入到租户中，作为第二种身份验证形式。 如果没有手机，仍然可以测试条件访问策略。 以 Adele Vance 的身份登录时，系统将要求你使用第二种身份验证形式登录。 此时，只需取消登录，然后以 Holly 的身份重新登录，Holly 不需要 MFA。 虽然你不会完成 MFA 登录，但你会验证系统在你尝试以 Adele 的身份登录时是否强制你使用 MFA。

1. 在 LON-CL1 VM 中，Microsoft 365 管理中心应延续之前的任务，仍在 Microsoft Edge 浏览器中打开。**** 你应以 Holly Dickson 的身份登录到 Microsoft 365。**** 首先从 Microsoft 365 退出登录。 在“Microsoft 365 管理中心”选项卡中，选择浏览器右上角的 Holly 姓名。**** 在显示的“Holly Dickson”窗口中，选择“退出登录”。******** <br/>
    
    **重要提示：** 退出登录后，关闭浏览器会话以清除缓存，然后打开新的 Microsoft Edge 浏览器会话。 
    
2. 选择任务栏中的 Edge 图标以打开新的浏览器会话。**** 在浏览器中，在地址栏中输入以下 URL 转到 Microsoft Office 主页：******https://portal.office.com/** 

3. 在“选取帐户”窗口中，选择“使用另一个帐户”。******** 

4. 在“登录”窗口中，输入 **AdeleV@xxxxxZZZZZZ.onmicrosoft.com**（其中 xxxxxZZZZZZ 是你的实验室托管提供商提供的租户前缀），然后选择“下一步”********。 在“输入密码”窗口中，输入实验室托管提供商为租户管理员帐户（即 MOD 管理员帐户）提供的“管理密码”，然后选择“登录”。********

5. 由于为除 M365 试点项目组成员（Adele 不是成员）之外的所有用户启用了 MFA，因此将显示“需要更多信息”窗口。**** 选择**下一步**。 这会返回到“Microsoft Authenticator”页面，这是使用 MFA 登录的起点。**** <br/>

    **重要提示：** 如果没有手机，那么在尝试以 Adele 身份登录时，这是你可以走到的最远一步。 即使无法完成登录，你也验证了条件访问策略的第一部分是否正常工作，因为它要求 Adele 使用 MFA 登录。 此时，关闭 Edge 浏览器会话，然后跳到步骤 #15，此时你将以 Holly 的身份重新登录。

6. 在显示的 Microsoft Authenticator 页面上，可以下载此移动应用或使用其他方法进行 MFA 验证。**** 出于本实验室的目的，我们建议你使用手机，这样就不必花时间安装你在此培训课程之后可能不会再次使用的 Microsoft Authenticator 应用。 选择在页面底部出现的“我想设置其他方法”选项（重要提示：******** 不要将此链接与其上方显示的“我想使用不同的验证器应用”混淆）。**** 

7. 在显示的“选择其他方法”对话框中，选择“你要使用哪种方法?”字段中的下拉箭头，选择“手机”，然后选择“确认”。**************** 

8. 在显示的“手机”窗口中，在“你要使用的手机号码是什么?”字段下，选择你的国家或地区，然后在它旁边的字段中输入你的电话号码（采用 nnn-nnn-nnnn 格式）。************ 验证你是否选择了“接收代码”选项，然后选择“下一步”。********
...
12. 如果出现“保持登录?”对话框，请选中“不再显示此内容”复选框，然后选择“是”************。 

13. 在“主页 | Microsoft 365”选项卡上，选择屏幕左侧应用图标列中显示的 Word 图标。******** 这将在新的浏览器选项卡中打开 Microsoft Word。**** 这样做可验证在使用 MFA 登录后是否可以访问 Microsoft 365 应用。  <br/>
...
14. 在“Microsoft 365 管理中心”选项卡上，选择浏览器右上角 Adele 帐户的图标。**** 在显示的“Adele Vance”窗口中，选择“退出登录”。******** <br/>
    
    **重要提示：** 退出登录后，关闭浏览器会话以清除缓存，然后打开新的 Microsoft Edge 浏览器会话。 
    
15. 选择任务栏中的 Edge 图标以打开新的浏览器会话。**** 在浏览器中，在地址栏中输入以下 URL 转到 Microsoft Office 主页：******https://portal.office.com/** 

16. 在“选取帐户”窗口中，选择 **Holly@xxxxxZZZZZZ.onmicrosoft.com**（其中 xxxxxZZZZZZ 是你的实验室托管提供商提供的租户前缀），然后选择“下一步”********。 在“输入密码”窗口中，输入实验室托管提供商为租户管理员帐户（即 MOD 管理员帐户）提供的相同“管理密码”，然后选择“登录”。********

17. 由于 MFA 是除 M365 试点项目团队成员（Holly 是成员）之外所有用户必需的，因此 Holly 不需要 MFA。 由于不需要 MFA，系统将显示“Microsoft 365 主页”。**** 选择管理员图标以转到 Microsoft 365 管理中心。******** <br/>

    **重要提示：** 你现已验证所创建的条件访问策略的第二部分是否正常工作。 该策略将 Microsoft 365 试点项目组的成员从使用 MFA 登录的要求中排除。 Holly 是此组的成员，因此她不必使用 MFA 登录。

18. 保持登录到 LON-CL1，保持浏览器中的 Microsoft 365 管理中心的打开状态。****
...
4. 需要编辑包含组织帐户锁定策略的组策略。 如有必要，在左侧窗格的根控制台树中，展开“Forest:Adatum.com”，然后依次展开“域”和“Adatum.com”。************  <br/>

    ‎在“Adatum.com”下，右键单击“默认域策略”，然后在菜单中选择“编辑”************。

5. 最大化出现的“组策略管理编辑器”窗口。****

6. 在左侧窗格中的“默认域策略”树中，在“计算机配置”下，展开“策略”，展开“Windows 设置”，展开“安全设置”，然后展开“帐户策略”。************************

7. 在“帐户策略”文件夹中，选择“帐户锁定策略”********。

8. 正如右侧窗格所示，没有定义任何智能锁定参数。 你无需在组策略管理编辑器中维护这些锁定参数，并将转而使用 Microsoft Entra 管理中心。 虽然你可以使用组策略管理编辑器，但此方法通常用于本地 Active Directory 环境。 我们向你展示此编辑器是为了让你了解此替代方法。 但是，对于严格使用基于云的服务（如 Microsoft 365）或觉得使用 Microsoft Entra 管理中心比访问组策略管理编辑器更方便的组织来说，使用 Microsoft Entra 管理中心在 Entra ID 上下文中分配相应的值是更好的选择。**** <br/>  
...
11. 在“输入密码”对话框中，输入实验室托管提供商提供的唯一管理密码，然后选择“登录”************。
...
14. 如果出现“发现更多应用”窗口或“使用 Microsoft 365 创建”窗口，请选择窗口右上角的“X”将其关闭************。 

15. 在“欢迎使用 Microsoft 365”页上，在左侧窗格中显示的应用程序图标列表中，选择“管理员”，随即会在新浏览器选项卡中打开 Microsoft 365 管理中心************。 
...
28. 在“输入密码”窗口中，输入任意字母的随机组合，然后选择“登录”。******** 请注意出现的密码无效错误消息。 
...
29. 在先前设置的“90 秒锁定持续时间”结束之前，你将被禁止以 Patti 的身份再次登录。**** <br/>
    ...
    在你被锁定后，请等待 90 秒，然后作为 **pattif@xxxxxZZZZZZ.onmicrosoft.com** 重新登录（其中 xxxxxZZZZZZ 是实验室托管提供商分配给你的租户前缀）。 在“密码”字段中，输入 Patti 的密码，该密码与实验室托管提供商为租户管理员帐户（即 MOD 管理员帐户）提供的管理密码相同。******** 验证你是否能够成功以 Patti 身份登录。
